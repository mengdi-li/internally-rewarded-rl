...
mnist_data = load_data('./data/MNIST/mnist.pkl.gz')
outfile = "./data/{}/mnist{}x{}_sequence1_{}dsistortions{}x{}".format("ClutteredMNIST" if NUM_DISTORTIONS > 0 else "TranslatedMNIST", 
            OUT_SHP[0], OUT_SHP[1], NUM_DISTORTIONS, dist_size[0], dist_size[1])

np.random.seed(1234)

X_train_valid = np.concatenate([mnist_data['X_train'],
                             mnist_data['X_valid']], axis=0) # 60000
y_train_valid = np.concatenate([mnist_data['y_train'],
                             mnist_data['y_valid']], axis=0) # 60000

### create list with distortions
all_digits = X_train_valid
all_digits = all_digits.reshape([-1] + ORG_SHP) # (60000,28,28)
...
def create_sample(x, output_shp, num_distortions=NUM_DISTORTIONS):
    ...
    for i,digit in enumerate(x):
        ...
        x_end = x_start + b
        y_start = y_offset + np.floor(i*angle).astype(int)
        y_end = y_start + a
...
def create_dataset(n, X, labels, org_shp, out_shp):
    ...
    for i in range(n):
        if (i+1) % 1000 == 0:
            print(i)
        x_, y_ = sample_digits(NUM_SEQUENCE, X, labels, org_shp)
...
samples = [create_sample(sample_digits(
    NUM_SEQUENCE, mnist_data['X_train'],
    mnist_data['y_train'],
...
X_train, y_train = create_dataset(N_TRAIN, X_train_valid,
                                  y_train_valid, ORG_SHP, OUT_SHP
                                  )
