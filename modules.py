...
class LocationNetwork(nn.Module):
    ...
    def forward(self, h_t):
        ...
        if self.training: # only do random sampling in training
            ...
            log_pi = torch.zeros(mu.shape[0], device=mu.device)
            l_t = mu 

        return log_pi, l_t

class LocationNetworkEarlyStopping(nn.Module):
    """The location network.
    ...
    def forward(self, h_t):
        # compute mean
        feat = F.relu(self.fc(h_t.detach()))
        output = self.fc_lt(feat)
        mu = torch.tanh(output[:,:2])
        ...
        if self.training: # only do random sampling in training
            ...
            try:
                ...
                print("value error") 
            l_t = l_t.detach()
            l_log_pi = Normal(mu, self.std).log_prob(l_t)

            # we assume both dimensions are independent
            # 1. pdf of the joint is the product of the pdfs
            # 2. log of the product is the sum of the logs
            l_log_pi = torch.sum(l_log_pi, dim=1)

            # bound between [-1, 1]
            l_t = torch.clamp(l_t, -1, 1)
        else:
            l_log_pi = torch.zeros(mu.shape[0], device=mu.device)
            l_t = mu 

        # probability of stopping
        s_pi = torch.sigmoid(output[:,2])
        s_pi = torch.clamp(s_pi, 0.001, 0.999)

        return l_t, l_log_pi, s_pi
...
class Plannernetwork(nn.Module):
    ...
    def forward(self, h_t):
        ...
        p_dist = torch.distributions.categorical.Categorical(p_t)
        action_or_stop = p_dist.sample()
        log_p = p_dist.log_prob(action_or_stop).squeeze()

        return log_p, action_or_stop