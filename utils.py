...
def set_requires_grad(modules, state):
...
def denormalize(T, coords):
    # return (0.5 * ((coords + 1.0) * T)).astype(int)
    return (0.5 * ((coords + 1.0) * T))


def bounding_box(x, y, size, linewidth=1.0, color="w", linestyle="-"):
    x = int(x - (size / 2)) # There two lines correspond to ``F.pad(...)` in modules.py. 
                            # We don't need to really get the patch here so we don't use `F.pad()` here.
    y = int(y - (size / 2))
    rect = patches.Rectangle(
        (x, y), size, size, linewidth=linewidth, edgecolor=color, fill=False, linestyle=linestyle
    )
    return rect
...
def plot_images(images, gd_truth):
    ...
    plt.show()
    plt.savefig("./data/dataset_vis.jpg")


def prepare_dirs(config):
    for path in [config.exp_dir, config.data_dir, config.ckpt_dir, config.logs_dir]:
